...
function Card({ onClose, id, name, status, species, gender, origin, image, addFav,removeFav,myFavorites}) {
   ...
   function navigateHandler(){navigate(`/detail/${id}`)}

   const [isFav,setIsFav]=useState(false);
   const handleFavorite =()=>{
      if(isFav){           //Si es favorito
         setIsFav(false);
         removeFav(id)
      }
      else{
         setIsFav(true);
         addFav({ onClose, id, name, status, species, gender, origin, image} )   //Se envía todos los datos del personaje
      }
   }
...
const mapDispatchToProps = (dispatch) => {         //Se están enviando estas dos funcione por props
   return{
      addFav:(character)=>{dispatch(addFav(character))},    //!SIEMPRE SE DISPTACH UN OBJETO, POR ESO SE ENVIA EJECUTANDO LA FUNCION ()
